...
# 이 Base를 상속받으면 클래스=테일블, 클래스 속성=컬럼으로 자동 매핑
Base = declarative_base()
# 테이블 재설계

Base = declarative_base() # 선언적 베이스 클래스 -> 자동으로 테이블과 매핑
...
class SubwayRaw(Base):
    ...
    시작시 = Column(Integer, nullable=False) # 시간대 컬럼에서 뽑아낸 시작 시각 (0~23) -> 시간대별 집계/정렬 사용
    날짜 = Column(Date, nullable=False) # 연-월-일 합쳐진 실제 날짜 타입
