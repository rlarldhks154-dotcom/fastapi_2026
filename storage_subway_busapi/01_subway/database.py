...
def init_db(drop_existing: bool = True):
    ...
    if drop_existing:
        ...
        print('[database] 기존 subway_raw 테이블 삭제(재설계을 위해)')
...
def get_session():
    ...
    *이 함수를 만드는 이유
    1. 호출부를 단순하게 유지 : 다른 모듈(파일)에서 SessionLocal이 뭔지 몰라도 from database import get_session만 세션을 바로 사용 할 수 있다.
    2. 나중에 코드가 바뀌어도 한 곳만 수정 : 나중에 세션 생성 시 로깅을 추가하거나, 그 외 기능을 수정 및 추가할 때 get_session() 내부만 수정하면 된다.
