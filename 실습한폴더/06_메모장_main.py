...
def home():
    """API 안내"""
    return {'message':'메모장 API입니다. /docs 에서 테스트 해보세요'}
...
def get_memo(memo_id: int):
    ...
    if memo is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f'{memo_id}번 메모를 찾을 수 없습니다.'
        )
    return memo
...
def update_memo(memo_id: int, body: MemoUpdate):
    """메모 수정 (부분 수정) - content만 보내면 content만 수정, updated_at은 수정 시각 (자동 갱신)"""
    memo = find_memo(memo_id)
    if memo is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f'{memo_id}번 메모를 찾을 수 없습니다'
