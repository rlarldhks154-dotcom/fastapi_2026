...
def setup_db():
    ...
    # teardown : 테스트가 끝난 뒤 정리(청소)하는 작업
...
def client(setup_db):
    ...
    # TestClient(app): 실제 서버 없이 app에 직접 HTTP 요청 가능
    with TestClient(app) as c:
        yield c # 테스트 함수에서 client라는 이름으로 이 c를 받아 사용
    
    # with 종료 후: dependency_overrides 초기화 --> 다른 테스트 파일에 영향 주지 않도록 원상 복귀
