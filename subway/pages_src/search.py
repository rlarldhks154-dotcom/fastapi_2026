...
목적 : 폼 안에 입력 위젯들(역 이름, 승하구분, 기간, 시작시 범위, 요일) --> 조건들을 조작할 때마다
        ...
        만약에 데이터가 많을 때 폼 없이 위젯을 조작하면 필터링을 다시 하면 체감 속도가 느려진다.
        --> st.form을 사용한다
...
with st.form('subway_search_form'):
    col1, col2 = st.columns(2) # 세로 2칸 (1:1비율)
    with col1:
    ...
    hour_range = st.slider('조회 시간대 (시작시 기준)',
                           ...
                           value=(5, 23))
    # 멀티 셀렉트 위젯
    weekday_pick = st.multiselect('요일', options=WEEKDAY_ORDER, default=WEEKDAY_ORDER)
...
if submitted:
    ...
    if keyword:
        # filtered[filtered['역명'].str.contains --> 문자열의 일부만도 검색하려고(포함하니?)
